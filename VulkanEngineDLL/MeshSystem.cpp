...
uint MeshSystem::CreateMesh(MeshTypeEnum meshType, Vector<Vertex2D>& vertexList, Vector<uint32>& indexList)
{
    ...
    Mesh mesh = Mesh
    {
        ...
        .MeshVertexBufferId = bufferSystem.VMACreateStaticVulkanBuffer(vertexList.data(), vertexList.size() * sizeof(Vertex2D), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT),
        .MeshIndexBufferId = bufferSystem.VMACreateStaticVulkanBuffer(indexList.data(), indexList.size() * sizeof(uint32), VK_BUFFER_USAGE_INDEX_BUFFER_BIT),
