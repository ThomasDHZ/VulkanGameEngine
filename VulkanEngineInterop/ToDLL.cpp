...
RenderPassNodeDLL* ToDLL_RenderPassNodeDLL(Vector<RenderPassNode>& renderPassNodeList, VkCommandBuffer& commandBuffer, float deltaTime, size_t* renderPassNodeCount)
{
    if (renderPassNodeCount) *renderPassNodeCount = 0;
    if (renderPassNodeList.empty()) return nullptr;
    ...
    for (size_t x = 0; x < renderPassNodeList.size(); ++x)
    {
        ...
        if (renderPassNodeCount) *renderPassNodeCount = renderPassNodeList.size();
        return dllList;
    }
}