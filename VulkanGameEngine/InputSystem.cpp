...
void InputSystem::Update(const float& deltaTime)
{
    ...
    {
        Vector<KeyState> keyBoardState(const_cast<KeyState*>(keyboard.GetKeyBoardState()), const_cast<KeyState*>(keyboard.GetKeyBoardState()) + MAXKEYBOARDKEY);
        input.KeyPressed = keyBoardState;
        int a = 342;
        //int joy = GLFW_JOYSTICK_1;
