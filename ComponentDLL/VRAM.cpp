...
Vector<Vector<uint>> VramSystem::LoadLevelLayout(const String& levelLayoutPath)
{
    ...
    nlohmann::json json = File_LoadJsonFile(levelLayoutPath.c_str());
...
//}
//
//Animation2DDLL VRAM_LoadSpriteAnimations(const char* spritePath, size_t& animationListCount)
//{
//    Vector<Animation2D> animationList;
//    nlohmann::json json = File_LoadJsonFile(spritePath);
//    for (size_t x = 0; x < json["AnimationList"].size(); ++x)
...
//    Animation2DDLL* animationListPtr = memorySystem.AddPtrBuffer<Animation2DDLL>(animationList.data(), animationList.size(), __FILE__, __LINE__, __func__, ("Sprite animation: " + String(spritePath)).c_str());
