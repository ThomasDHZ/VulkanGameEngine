...
//#endif
  ...
  void LevelSystem::Draw(VkCommandBuffer& commandBuffer, const float& deltaTime, VulkanRenderPass vulkanRenderPass, VulkanPipeline vulkanPipeline)
  {
      ...
      uint32 fbIndex = renderPass.IsRenderedToSwapchain ? vulkanSystem.ImageIndex : 0;
      VkRenderPassBeginInfo renderPassBeginInfo = VkRenderPassBeginInfo
      {
          .sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO,
          .renderPass = renderPass.RenderPass,
            .framebuffer = renderPass.FrameBufferList[fbIndex],
          .renderArea = VkRect2D
          {
  ...
  void LevelSystem::RenderPrefilterMapRenderPass(VkCommandBuffer& commandBuffer, VkGuid& renderPassId, float deltaTime)
  {
      ...
      for (uint32_t mip = 0; mip < prefilterMipmapCount; ++mip)
      {
          ...
          vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(indiceList.size()), 1, 0, 0, 0);    
          vkCmdEndRenderPass(commandBuffer);
      }
