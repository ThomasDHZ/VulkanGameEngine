...
layout(location = 1) out vec4 outNormalData;         //Normal/NormalStrength                                                                      - R16G16B16A16_UNORM
...
layout(push_constant) uniform materialCreaterBuffer 
{
    int MeshBufferIndex;
} materialCreater;
...
void main()
{
    int meshIdx = materialCreater.MeshBufferIndex;
    ...
    vec4  albedo                    = (material.AlbedoMap                    != 0xFFFFFFFFu) ? textureLod(TextureMap[material.AlbedoMap],                    UV, 0.0f)                 : vec4(material.Albedo, material.Alpha);
    ...
    vec3  normalMap                 = (material.NormalMap                    != 0xFFFFFFFFu) ? textureLod(TextureMap[material.NormalMap],                    UV, 0.0f).xyz * 2.0 - 1.0 : vec3(0.0f, 0.0f, 1.0f) * 2.0 - 1.0;
    float alpha                     = (material.AlphaMap                     != 0xFFFFFFFFu) ? textureLod(TextureMap[material.AlphaMap],                     UV, 0.0f).r               : material.Alpha;
