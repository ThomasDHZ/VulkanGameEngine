...
//    vec2 finalUV = ParallaxOcclusionMapping(UV, viewDirTS, material.NormalDataId);
    ...
    vec4 albedoData         = textureLod(TextureMap[material.AlbedoDataId],         UV, 0.0f).rgba;    
//    vec4 normalData         = textureLod(TextureMap[material.NormalDataId],         UV, 0.0f).rgba;    
//    vec4 packedMROData      = textureLod(TextureMap[material.PackedMRODataId],      UV, 0.0f).rgba;   
//    vec4 packedSheenSSSData = textureLod(TextureMap[material.PackedSheenSSSDataId], UV, 0.0f).rgba;    
//    vec4 tempMapData        = textureLod(TextureMap[material.UnusedDataId],         UV, 0.0f).rgba;    
//    vec4 emissionData       = textureLod(TextureMap[material.EmissionDataId],       UV, 0.0f).rgba;    
