...
void main()
{
    CubeMapMaterial cubeMapMaterial = GetCubeMapMaterial(gBufferSceneDataBuffer.CubeMapMaterialIndex);
    ...
    if (depth >= 0.9999f) 
    {
        ...
        vec3 sky = textureLod(CubeMaps[cubeMapMaterial.CubeMapId], worldDir, 0.0).rgb;
    ...
    vec3 ambient = ImageBasedLighting(F0, V, R, material, cubeMapMaterial);
...
vec3 ImageBasedLighting(vec3 F0, vec3 V, vec3 R, Material material, CubeMapMaterial cubeMapMaterial)
{
    ...
    vec3  irradiance = texture(CubeMaps[cubeMapMaterial.IrradianceMapId], material.Normal).rgb;
    vec3  diffuseIBL = (material.Albedo / PI) * irradiance;

    float maxLod = textureQueryLevels(CubeMaps[cubeMapMaterial.PrefilterMapId]) - 1.0;
    float lod = material.Roughness * maxLod;
    lod = clamp(lod, 0.0, maxLod);
    vec3  prefilteredColor = textureLod(CubeMaps[cubeMapMaterial.PrefilterMapId], R, lod).rgb;

    vec2  brdf = texture(TextureMap[gBufferSceneDataBuffer.BRDFMapIndex], vec2(max(dot(material.Normal, V), 0.0f), material.Roughness)).rg;
    vec3  specularIBL = prefilteredColor * (F * brdf.x + brdf.y);

    float iblSheenFactor = pow(1.0 - max(dot(material.Normal, V), 0.0f), 5.0);
    vec3  iblSheenContrib = sheenColor * material.SheenIntensity * iblSheenFactor * irradiance;

    float subsurfaceStrength = 0.7f;
    vec3  iblSSSContrib = subsurfaceStrength * irradiance * (material.Albedo * material.SubSurfaceScattering);

    float coatNdotV = max(dot(material.Normal, V), 0.0f);
    vec3  coatR = reflect(-V, material.Normal);
    float coatLod = clearcoatRoughness * (textureQueryLevels(CubeMaps[cubeMapMaterial.PrefilterMapId]) - 1.0);
    coatLod = clamp(coatLod, 0.0, textureQueryLevels(CubeMaps[cubeMapMaterial.PrefilterMapId]) - 1.0);

    vec3  coatPrefilter = textureLod(CubeMaps[cubeMapMaterial.PrefilterMapId], coatR, coatLod).rgb;

    vec2  coatBRDF = texture(TextureMap[gBufferSceneDataBuffer.BRDFMapIndex], vec2(coatNdotV, clearcoatRoughness)).rg;
